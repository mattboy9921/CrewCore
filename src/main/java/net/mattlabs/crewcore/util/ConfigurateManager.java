...
public class ConfigurateManager {
    ...
    public ConfigurateManager() {
        ...
        CrewCore.getInstance().getDataFolder().mkdir();
    }
    ...
    public <T> void add(String fileName, TypeToken<T> typeToken, T configSerializable, Supplier<T> configSerializableSupplier, UnaryOperator<ConfigurationOptions> configurationOptions) {
        File file = new File(CrewCore.getInstance().getDataFolder(), fileName);
    ...
    public <T> void saveDefaults(String fileName) {
        ...
        if (!file.exists()) {
            CrewCore.getInstance().getLogger().info("\"" + fileName + "\" file doesn't exist, creating...");
            try {
                loader.save(loader.createNode().set(configNode.getTypeToken(), configNode.getConfigSerializable()));
            }
            catch (IOException e) {
                CrewCore.getInstance().getLogger().severe("Failed to save \"" + fileName + "\"!");
                CrewCore.getInstance().getPluginLoader().disablePlugin(CrewCore.getInstance());
            }
        }
    }
    ...
    public <T> T load(String fileName) {
        ...
        try {
            ...
            CrewCore.getInstance().getLogger().severe("Failed to load \"" + fileName + "\" - using a default!");
        }
        return t;
    }
